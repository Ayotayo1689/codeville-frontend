...
import result from "../../../assets/caseStudy/result.png";
import { BsArrowRight, BsTelephone } from "react-icons/bs";
import { cards, ourCabilitiesTabs, overviewTabs } from "./data";
import {
	Accordion,
	AccordionButton,
	AccordionIcon,
...
const SingleCaseStudy = ({ data, projects }) => {
			...
			<Header>
				<Hide below="md">
					...
					>
						...
						<BreadcrumbItem>
							<BreadcrumbLink as={Link} href={"/case-studies"} color={"white"}>
								Case Study
							</BreadcrumbLink>
						</BreadcrumbItem>
						<BreadcrumbItem>
								...
								<Show below="md">
									...
									<Accordion defaultIndex={[0]} allowMultiple my={"2rem"}>
										<AccordionItem my={"1rem"}>
											...
											<Flex flexWrap={"wrap"}>
												{ourCabilitiesTabs.map((icon, index) => {
													return (
						...
						>
							{relatedProjects.slice(0, 3).map((props) => {
								return (
									...
									>
										<Link
											href={`${appRoutes.caseStudies}${props?.slug?.replace(
												/_/g,
												"-"
											)}/${props.id}`}
