...
import {Container, Text, Box, Flex, Image, Icon} from "@chakra-ui/react";
...
const HealthCareForms = ({ id }) => {
	const index = Number(id) || 0;
	return (
		<Box>
			<Container
				maxW={"7xl"}
				jusgtifyContent={"center"}
				py={"3rem"}
				pb={"2rem"}
			>
				...
				>
					...
					>
						...
						{formList[index].items.map((item, index) => {
							return (
								<Flex key={index} gap={"3"} pb={"1rem"} alignItems={"center"}>
									<div><FaCheck color={"red"}/></div>
					...
					<Box p={{ base: "1rem", lg: "2rem 4rem" }}>
						<Flex>
							...
							<Box>
								<Image src={"/assets/healthcare/icon1.png"} alt={"icon"} width={300}/>
							</Box>
						</Flex>
						<Box>
							<ContactUsForm
								placeholder={
									formList[index]?.placeholder ||
									"I’m interested in IT support team augmentation for my healthcare organization."
								}
								submit={formList[index]?.summit || "START A DIALOG"}
							/>
						</Box>
					</Box>
				</Flex>
